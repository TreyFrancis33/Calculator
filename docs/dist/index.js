...
window.addEventListener("keydown", (e) => {
  ...
  if (e.key === "/" || e.key === "*" || e.key === "+" || e.key === "-") startOperation(operatorButton);
  if (e.key === ".") decimate();
...
operatorButtons.forEach((button) => {
	button.addEventListener("click", () => {startOperation(button);});
});
...
decimalButton.addEventListener("click", () => {decimate();});
...
function completeOperation(operation) {
  ...
  if (operation.toString().includes(".")) {
    ...
    if (firstTwo === "00") {operationContainer.innerHTML = toFixed(operation, 4);} 
		else {operationContainer.innerHTML = toFixed(operation, 2);}
  } else {operationContainer.innerHTML = operation.toString();}
  justOperated = true;
}
...
function concatNums(operators2, negated2, decimated) {
  let concat = "";
  for (let i = 0; i < operators2.length; i++) concat += operators2[i].toString();
  if (negated2) return `-${concat}`;
  if (decimated) return `${operators2[0]}.${operators2[1]}`;
  return concat;
}
...
function disabledButtons(enableOrDisable) {
  numberButtons.forEach((button) => {button.disabled = enableOrDisable;});
  operatorButtons.forEach((button) => {button.disabled = enableOrDisable;});
  const btns = [squareRootButton, negateButton, squareButton, decimalButton, equalButton];
  for (let i = 0; i < 5; i++) {btns[i].disabled = enableOrDisable;}
}
